...
def add_to_watchlist(movie_id):
    ...
    watchlist_item = Watchlist.query.filter_by(user_id=user.id, movie_id=movie_id).first()
    if not watchlist_item:
        # Add movie to watchlist
        watchlist_item = Watchlist(user_id=user.id, movie_id=movie_id)
        db.session.add(watchlist_item)
        db.session.commit()
        message = "Movie added to watchlist!"
    else:
        message = "Movie already in watchlist."
    return render_template('movie_details.html', user=current_user, flash=message)
